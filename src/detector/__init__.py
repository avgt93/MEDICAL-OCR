...
def _load_model():
    ort_session = ort.InferenceSession(
        resource_filename("src", "models/detector.onnx"),
...
def detect(img: np.ndarray, scale: float, margin: int) -> List[DetectorRes]:
    ...
    aabbs = decode(
        pred_map, comp_fg=fg_by_cc(0.5, 100), f=img_batch.shape[2] / pred_map.shape[1]
    )
    aabbs = [
        aabb.scale(1 / scale, 1 / scale)
        for aabb in aabbs
...
def _cluster_lines(
    detections: List[DetectorRes], max_dist: float = 0.7, min_words_per_line: int = 2
    ...
    dbscan = DBSCAN(
        eps=max_dist, min_samples=min_words_per_line, metric="precomputed"
    ).fit(dist_mat)
    ...
    res = sorted(
        clustered.values(),
        key=lambda line: [det.aabb.ymin + det.aabb.height / 2 for det in line],
    )
    return res
