...
from .ctc_and_beam import ctc_best_path, ctc_single_word_beam_search, PrefixTree


def _load_model():
    ort_session = ort.InferenceSession(
        resource_filename("src", "models/reader.onnx"),
        providers=["CUDAExecutionProvider", "CPUExecutionProvider"],
    )
    with open(resource_filename("src", "models/reader.json")) as f:
...
def read(
    img: np.ndarray, decoder: str, prefix_tree: Optional[PrefixTree] = None
) -> str:
    img = transform(img)
    ...
    if decoder == "best_path":
        ...
        raise Exception(
            'Unknown decoder. Available: "best_path" and "word_beam_search".'
        )
